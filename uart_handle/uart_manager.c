...
static void push_to_tx_queue(uint8_t *data, uint16_t length)
{
    ...
    {
        SEGGER_RTT_WriteString(0, "UART TX queue overflow. Some data may be lost.\r\n");
    }
}
...
void uart_manager_send_data( uint8_t *data, uint16_t length)
{
    ...
    {
        SEGGER_RTT_WriteString(0, "UART TX queue is busy. Data not sent.\r\n");
    }
}
...
{
    // handle rx queue
    while (queue_pop_byte(&uart_rx_queue, &uart_rx_buffer[0]) == QUEUE_SUCCESS)
    {
        uart_parser_unpack_data_byte(uart_rx_buffer[0], uart_rx_buffer, sizeof(uart_rx_buffer));
    }
    //
    if (queue_get_data_length(&uart_tx_queue) > 0)
    {
        ...
        SEGGER_RTT_WriteString(0, "Sending data from UART TX queue...\r\n");
    }
}
